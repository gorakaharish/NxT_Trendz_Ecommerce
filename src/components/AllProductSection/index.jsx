...
const AllProductsSection = () => {
  ...
  const renderLoadingView = () => (
    <div className="products-loader-container">
      <ThreeCircles  type="ThreeDots" color="#0b69ff" height="100" width="100" />
    </div>
  )
  ...
  const renderProductsListView = () => {
    return productsList.length > 0 ? (
      <div className="all-products-container">
        <ProductsHeader activeOptionId={activeOptionId} sortbyOptions={sortbyOptions} changeSortby={setActiveOptionId} />
        <ul className="products-list">
          {productsList.map(product => <ProductCard productData={product} key={product.id} />)}
        </ul>
      </div>
    ) : (
      <div className="no-products-view">
        <img src="https://assets.ccbp.in/frontend/react-js/nxt-trendz/nxt-trendz-no-products-view.png" className="no-products-img" alt="no products" />
